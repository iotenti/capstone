...
function checkUserCustomer($db, $customer_username, $password, $hash) { // checks if the user is a customer
...
function checkUserEmployee($db, $username, $password, $hash) { // checks if the user entered is an employee
...
function updateInventory($db, $inventory){ // function that updates inventory
    try{
        $update = $inventory;
        $carId = $update['car_id'];
        ...
        $sql = $db->prepare("UPDATE inventory SET vendor_id=:vendor_id, vin_num=:vin_num, trim=:trim, 
                                ...
                                price=:price, description=:description, model=:model WHERE car_id='$carId'");
...
function deleteInventory($db, $carId){ // deletes a piece of inventory
    try{
...
function getVendorsAsTable($db){ // grabs vendors and displays them as a table
    try{
        $sql = "SELECT * FROM vendors";
        $sql = $db->prepare($sql);
        $sql->execute(); //executes statement
...
function validate($firstName, $lastName, $username, $password) { // checks to make sure each of the arguments are filled
    $ret = 0;
    if($firstName == "") {
        $ret = 1;
    }
...
function StoreNewUser($db, $customer_fname, $customer_lname, $customer_email, $customer_phone, $customer_username, $customer_password) { // adds new user to the db
...
function phoneCheck ($phone) { //checks to make sure the phone number entered is a valid phone number
...
function addEmployee($db, $f_name, $l_name, $email, $phone_number, $position, $username, $password0, $photo) { // adds a new employee to the db
...
function getCredentials($db, $employee_id) { // gets information about the employees from the db
...
function getTheTeam($db) { // displays the employees
...
function addVendor($db, $vendor){ // adds vendor to db
    try{
        $add = $vendor;
...
function updateVendor($db, $vendor){ // updates vendor to db
    try{
        $update = $vendor;
        $vendorId = $update['vendor_id'];
...
function deleteAVendor($db, $vendorId){ // deletes vendor
    try{
...
function getMessage($result){ // outputs the argument in an easy to read and styled manner
...
function getEmployeeNames($db) { // goes into db and grabbs employees names
    $sql = $db->prepare("SELECT * FROM employees");
    $sql->execute();
...
function scheduleApp($db, $employee_id, $customer_id, $appointment) { // adds a row to appointment table
    $approve = 0;
...
function customerApp($db, $customer_id) { // displays the appointments based on customer ID
...
function myAppointments($db, $employee_id) { // displays the appointments for employees
...
function getCustomer($db, $customer_id) { // gets customer info
...
function addInventory($db, $inventory){ // adds a car to the inventory
    try{
        $add = $inventory;
        ...
        $sql = $db->prepare("INSERT INTO `inventory`(`car_id`, `vendor_id`, `vin_num`, `trim`, `year`, `mileage`, `fuel_type`, `engine_type`, `transmission`, `mpg`, `color`, `drive_train`, `type_of_car`, `date_of_arrival`, `price`, `description`, `model`) VALUES (null, :vendorId, :vinNum, :inventoryTrim, :inventoryYear, :mileage, :fuelType, :engineType, :transmission, :mpg, :color, :driveTrain, :typeOfCar, NOW(), :price, :description, :model)");
...
function getVendorsList($db) { // gets the vendors as a list
...
function getCarTypesList($db) { // grabs the list of the types of cars
...
function search($brands, $types, $locate) { // container for sorting and searching the inventory table
...
function showCarInfo($db, $car_id, $from) { // shows the car information
...
function customershowCarInfo($db, $car_id, $from) { // shows the information for a car from the customer side of the db
    $sql = $db->prepare("SELECT inventory.car_id as car_id, inventory.vendor_id as vendor_id, inventory.trim as trim, inventory.year as year, inventory.mileage as mileage, inventory.fuel_type as fuel, inventory.model as model,
 inventory.price as price, vendors.vendor_id, vendors.vendor_name as vendor, inventory.mpg as mpg, inventory.engine_type as engine, 
 inventory.color as color, inventory.type_of_car as type, inventory.vin_num as vin_num, inventory.transmission as trans, inventory.drive_train as drive_train, inventory.description as description
    FROM inventory
...
function addImages($db, $pk, $imagePath){ // adds an image to the db
    try{
